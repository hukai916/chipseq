//
// Deduplicate multi-mapped reads in a BAM file
//
...
workflow BAM_MARKDUPLICATES_MULTIMAPPER {
    ...
    main:
    DEDUP_MULTIMAPPER ( ch_reads, ch_fasta, ch_fai )

    ch_markdup = DEDUP_MULTIMAPPER.out.bam.mix(DEDUP_MULTIMAPPER.out.cram)
    ...
    emit:
    bam      = DEDUP_MULTIMAPPER.out.bam     // channel: [ val(meta), path(bam) ]
    cram     = DEDUP_MULTIMAPPER.out.cram    // channel: [ val(meta), path(cram) ]
    metrics  = DEDUP_MULTIMAPPER.out.metrics // channel: [ val(meta), path(metrics) ]
